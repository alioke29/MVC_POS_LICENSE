...
namespace RINOR_POS.Controllers
{
    ...
    /// Product Combo Link
    /// </summary>
    public class APIProductComboLinkController : ApiController
    {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_product_combo.Where(a => a.ShopID == MasterShopID && (a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate))
                                join b in db.pos_product_combo_link on a.ProductComboID equals b.ProductComboID
                                select b).ToList();
                ...
                {
                    var _qry = (from a in db.pos_product_combo_link
                                join b in db.pos_product_combo.Where(b => b.ShopID == MasterShopID) on a.ProductComboID equals b.ProductComboID
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
