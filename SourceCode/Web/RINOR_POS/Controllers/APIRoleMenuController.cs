...
namespace RINOR_POS.Controllers
{
    ...
    /// API Role Menu
    /// </summary>
    public class APIRoleMenuController : ApiController
    {
        ...
        public HttpResponseMessage Get(string KeyToken = "", int ShopId = 0, double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_role_menu.Where(a => a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate)
                                join b in db.pos_user_role.Where(b => b.ShopID == ShopId) on a.UserRoleID equals b.UserRoleID
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_user_role
                                where a.ShopID == ShopId
                                join b in db.pos_role_menu on a.UserRoleID equals b.UserRoleID
                                select b).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
