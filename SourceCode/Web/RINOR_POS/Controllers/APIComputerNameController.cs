...
{
    /// <summary>
    /// API For Data Computer Name
    /// </summary>
    public class APIComputerNameController : ApiController
    {
        private ModelPOSDB db = new ModelPOSDB();

        /// <summary>
        /// Looks up some data Computer Name.
        /// </summary>
        ...
        public HttpResponseMessage Get(string KeyToken = "", int ShopID = 0, double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_computer_name.Where(a => a.ShopId == ShopID && (a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate))
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_computer_name.Where(a => a.ShopId == ShopID)
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
