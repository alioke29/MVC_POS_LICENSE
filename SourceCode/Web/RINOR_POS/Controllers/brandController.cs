...
namespace RINOR_POS.Controllers
{
    ...
    public class brandController : BaseController
    {
        ...
        public ActionResult Create([Bind(Include = "BrandID, BrandKey, MerchantID, BrandCode, BrandName, IPAddress, Activate, ValidDateStart, ValidDateEnd")] brandViewModel branddata)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        client.BaseAddress = new Uri(UserProfile.APIURLBase + "/api/APIRegisterBrand?KeyToken=uptome");

                        //HTTP POST
                        var result = client.PostAsJsonAsync("", brand).Result;

                        if (result.IsSuccessStatusCode)
                        {
                            brand.BrandKey = "Waiting for registration key";
        ...
        public ActionResult Edit([Bind(Include = "BrandID, BrandKey, MerchantID, BrandCode, BrandName, IPAddress, Activate, ValidDateStart, ValidDateEnd")] brandViewModel branddata)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    //brand.BrandKey = branddata.BrandKey;
                    brand.MerchantID = branddata.MerchantID;
                    brand.BrandCode = branddata.BrandCode;
                    brand.BrandName = branddata.BrandName;
                    brand.IPAddress = branddata.IPAddress;
        ...
        /// POST: brand/SendBrand/5
        [HttpGet]
        public JsonResult ResendBrand(int id)
        {
            pos_brand_data brandData = db.pos_brand_data.Find(id);
            if (brandData != null)
            {
                //Request Register to License
                using (var client = new HttpClient())
                {
                    ...
                    //HTTP POST
                    var result = client.PostAsJsonAsync("", brandData).Result;

                    if (result.IsSuccessStatusCode)
                    {
                        brandData.BrandKey = "Waiting for registration key";
                        db.Entry(brandData).State = EntityState.Modified;
                        db.SaveChanges();
                        return Json(1, JsonRequestBehavior.AllowGet);
                    }
        ...
        /// POST: brand/GetBrandKey/5
        [HttpGet]
        public JsonResult GetBrandKey(int id)
        {
            pos_brand_data brandData = db.pos_brand_data.Find(id);
            if (brandData != null)
            {
                ...
                using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri(UserProfile.APIURLBase + "/api/APIBrandByID?KeyToken=uptome&BrandID=" + id);
                    ...
                    if (result.IsSuccessStatusCode)
                    {
                        ...
                        pos_brand_data APIData = JsonConvert.DeserializeObject<pos_brand_data>(data);

                        if (APIData != null)
                        {
                            pos_brand_data brand = db.pos_brand_data.Find(APIData.BrandID);
                            brand.BrandKey = APIData.BrandKey;
                            db.Entry(brand).State = EntityState.Modified;
                            db.SaveChanges();
                        }
                    }
