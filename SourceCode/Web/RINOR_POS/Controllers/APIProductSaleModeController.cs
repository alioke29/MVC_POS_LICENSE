...
{
    /// <summary>
    /// API For Data Product Salemode
    /// </summary>
    public class APIProductSaleModeController : ApiController
    {
        private ModelPOSDB db = new ModelPOSDB();

        /// <summary>
        /// Looks up some data Product Salemode.
        /// </summary>
        ...
        public HttpResponseMessage Get(string KeyToken = "", int ShopID = 0, double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_product_salemode
                                join p in db.pos_products.Where(a => a.ShopID == ShopID && (a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate)) 
                ...
                {
                    var _qry = (from a in db.pos_product_salemode
                                join p in db.pos_products.Where(a => a.ShopID == ShopID )
                                on a.ProductID equals p.ProductID
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
