...
namespace RINOR_POS.Controllers
{
    ...
    /// Api For Product VAT
    /// </summary>
    public class APIProductVatController : ApiController
    {
        ...
        public HttpResponseMessage Get(string KeyToken = "", double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_product_vat.Where(a => a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate)
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_product_vat
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
