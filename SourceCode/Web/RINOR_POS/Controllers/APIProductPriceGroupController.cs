...
namespace RINOR_POS.Controllers
{
    ...
    /// API For Data City
    /// </summary>
    public class APIProductPriceGroupController : ApiController
    {
        private ModelPOSDB db = new ModelPOSDB();

        /// <summary>
        /// Looks up some data Product Price Group.
        /// </summary>
        ...
        public HttpResponseMessage Get(string KeyToken = "", double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_product_price_group.Where(a => a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate)
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_product_price_group
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
