...
namespace RINOR_POS.Controllers
{
    ...
    public class user_accountController : BaseController
    {
        ...
        public JsonResult List(string sidx, string sord, int page, int rows, bool _search, string searchField, string searchOper, string searchString)
        {
            ...
            var _list = (from u1 in db.pos_user_application
                         where (u1.deleted_date == null)
                         join e in db.pos_employee on u1.employee_id equals e.employee_id
                         join u2 in db.pos_user_application on u1.updated_by equals u2.user_id
                         into t_joined
                         from row_join in t_joined.DefaultIfEmpty()
        ...
        public ActionResult ModalFrm(int? id = 0)
        {
            ...
            int user_id = (int)id;
            pos_user_application ms_user = db.pos_user_application.Find(user_id);
            if (ms_user == null)
            {
            ...
            else
            {
                usr_acc = new user_accountViewModel()
                {
                    ...
                    employee_id = ms_user.pos_employee.employee_id,
                    employee_nik = ms_user.pos_employee.employee_nik,
                    employee_name = ms_user.pos_employee.employee_name,
                    employee_email = ms_user.pos_employee.employee_email
                };
        ...
        public JsonResult GetEmployeeList(int? id = 0)
        {
            ...
            if (employee_id == 0)
            {
                var _list = from t in db.pos_employee
                            where t.fl_active == true && t.deleted_date == null && !(from u in db.pos_user_application select u.employee_id).ToList().Contains(t.employee_id)
                            select new
                            {
                                t.employee_id,
                                t.employee_nik,
            ...
            {
                var _list = from t in db.pos_employee
                            where t.fl_active == true && t.deleted_date == null && t.employee_id == employee_id && !(from u in db.pos_user_application select u.employee_id).ToList().Contains(t.employee_id)
                            select new
                            {
                                t.employee_id,
                                t.employee_nik,
        ...
        public ActionResult Create([Bind(Include = "user_id,user_name,user_password,employee_id,fl_active")] user_accountViewModel user_acc)
        {
            if (ModelState.IsValid)
            {
                using (var transaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        pos_employee emp = db.pos_employee.Find(user_acc.employee_id);
                        if (emp == null)
                        {
                        ...
                        else
                        {
                            ...
                            pos_user_application ms_user = new pos_user_application()
                            {
                                user_name = user_acc.user_name,
                                user_password = user_acc.user_password,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "user_id,user_name,user_password,employee_id,fl_active")] user_accountViewModel user_acc)
        {
            if (ModelState.IsValid)
            {
                pos_user_application ms_user = db.pos_user_application.Find(user_acc.user_id);
                ms_user.user_name = user_acc.user_name;

                if (!string.IsNullOrWhiteSpace(user_acc.user_password))
                    ms_user.user_password = user_acc.user_password;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult SaveDataUserAccount([Bind(Include = "user_id,user_name,user_password,employee_id,fl_active")] user_accountViewModel user_acc)
        {
            if (ModelState.IsValid)
            {
                using (var transaction = db.Database.BeginTransaction())
