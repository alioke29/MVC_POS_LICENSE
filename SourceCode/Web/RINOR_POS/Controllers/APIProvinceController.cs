...
{
    /// <summary>
    /// API For Data Province
    /// </summary>
    public class APIProvinceController : ApiController
    {
        ...
        public HttpResponseMessage Get(string KeyToken = "", double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_province.Where(a => a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate)
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_province
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
