...
namespace RINOR_POS.Controllers
{
    ...
    public class APIProductTypeController : ApiController
    {
        ...
        /// <summary>
        /// Looks up some data Product Type.
        /// </summary>
        ...
        public HttpResponseMessage Get(string KeyToken = "", double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_product_type.Where(a => a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate)
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_product_type
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
