...
{
    /// <summary>
    /// API For Promo Buy Get
    /// </summary>
    public class APIPromoBuyGetController : ApiController
    {
        ...
        public HttpResponseMessage Get(string KeyToken = "", double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_promotion_buy_get.Where(a => a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate)
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_promotion_buy_get
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
