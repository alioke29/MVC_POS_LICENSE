...
namespace RINOR_POS.Controllers
{
    ...
    /// Payment Group Image
    /// </summary>
    public class APIPaymentGroupImageController : ApiController
    {
        ...
        /// <param name="PayGroupTypeID">The ID of the data.</param>
        public HttpResponseMessage Get(string KeyToken = "", int PayGroupTypeID = 0)
        {
            db.Configuration.ProxyCreationEnabled = false;

            if (Token.isValidToken(KeyToken))
            {
                pos_payment_group _qry = new pos_payment_group();
                _qry = (from a in db.pos_payment_group.Where(a => a.PayGroupID == PayGroupTypeID) select a).FirstOrDefault();
