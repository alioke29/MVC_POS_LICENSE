...
{
    /// <summary>
    /// API For Data Price Group Shop
    /// </summary>
    public class APIPriceGroupShopController : ApiController
    {
        private ModelPOSDB db = new ModelPOSDB();

        /// <summary>
        /// Looks up some data Shop by ShopID.
        /// </summary>
        ...
        public HttpResponseMessage Get(string KeyToken, int ShopID = 0)
        {
            ...
            {
                var _qry = (from a in db.pos_price_group_shop.Where(a => a.ShopId == ShopID)
                            select a).ToList();
