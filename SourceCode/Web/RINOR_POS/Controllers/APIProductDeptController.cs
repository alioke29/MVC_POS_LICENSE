...
{
    /// <summary>
    /// API For Data Product Department
    /// </summary>
    public class APIProductDeptController : ApiController
    {
        private ModelPOSDB db = new ModelPOSDB();

        /// <summary>
        /// Looks up some data Product Department.
        /// </summary>
        ...
        public HttpResponseMessage Get(string KeyToken = "", int ShopID = 0, double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_product_dept.Where(a => a.ShopID == ShopID && (a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate))
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_product_dept.Where(a => a.ShopID == ShopID) select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
