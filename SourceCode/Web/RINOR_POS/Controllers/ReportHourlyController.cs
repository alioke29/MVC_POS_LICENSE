...
namespace RINOR_POS.Controllers
{
    ...
    /// </summary>
    [Authorize]
    public class ReportHourlyController : BaseController
    {
        ...
        public JsonResult GetIndexList(string StartPeriod, string EndPeriod, string ShopId = "", string ShopList = "")
        {
            ...
            try
            {
                ...
                cmd.CommandText = "Exec usp_report_hourly @StartDate, @EndDate, @MasterShopID, @ShopID";
                ...
                conn.Open();
                List<ReportHourly> HourlyList = new List<ReportHourly>();
                using (SqlDataReader row = cmd.ExecuteReader())
                {
                    ...
                    while (row.Read())
                    {
                        ReportHourly hourly = new ReportHourly();
                        hourly.Hour = row["Hour"].ToString();
                        hourly.Bill = int.Parse(row["Bill"].ToString());
                        hourly.Customers = int.Parse(row["Customers"].ToString());
                        hourly.TotalPrice = decimal.Parse(row["TotalPrice"].ToString());
                        hourly.SubTotal = decimal.Parse(row["Subtotal"].ToString());
                        hourly.TotalDisc = decimal.Parse(row["TotalDisc"].ToString());
                        hourly.TotalSC = decimal.Parse(row["TotalSC"].ToString());
                        hourly.TotalSale = decimal.Parse(row["TotalSale"].ToString());
                        hourly.TotalVAT = decimal.Parse(row["TotalVAT"].ToString());
                        hourly.Rounding = decimal.Parse(row["Rounding"].ToString());
                        hourly.TotalPayment = decimal.Parse(row["TotalPayment"].ToString());
                ...
                return Json(new { data = HourlyList }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
