...
{
    /// <summary>
    /// API For Data Receipt Template Image
    /// </summary>
    public class APIReceiptTemplateImageController : ApiController
    {
        private ModelPOSDB db = new ModelPOSDB();

        /// <summary>
        /// Looks up some data Receipt Tamplate Image.
        /// </summary>
        ...
        public HttpResponseMessage Get(string KeyToken = "", int ShopID = 0, double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_receipt_template_image.Where(a => a.ShopID == ShopID && (a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate))
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_receipt_template_image.Where(a => a.ShopID == ShopID)
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
