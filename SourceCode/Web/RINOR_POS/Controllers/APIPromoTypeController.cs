...
{
    /// <summary>
    /// API For Promotion Type
    /// </summary>
    public class APIPromoTypeController : ApiController
    {
        private ModelPOSDB db = new ModelPOSDB();

        /// <summary>
        /// Looks up some data Promotion Type.
        /// </summary>
        ...
        public HttpResponseMessage Get(string KeyToken = "", double LastSycn = 0)
        {
            ...
            if (Token.isValidToken(KeyToken))
            {
                if (LastSycn > 0)
                {
                    ...
                    var _qry = (from a in db.pos_promotion_type.Where(a => a.CreatedDate >= lastsycnDate || a.UpdatedDate >= lastsycnDate || a.DeletedDate >= lastsycnDate)
                                select a).ToList();
                ...
                {
                    var _qry = (from a in db.pos_promotion_type
                                select a).ToList();
                    var json = JsonConvert.SerializeObject(_qry);
