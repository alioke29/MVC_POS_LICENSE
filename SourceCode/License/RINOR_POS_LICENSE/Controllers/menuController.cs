...
namespace RINOR_POS.Controllers
{
    ...
    public class menuController : Controller
    {
        ...
        public ActionResult GetMenuList()
        {
            try
            {
                var result = (from m in db.pos_menu_application
                              join n in db.pos_module_application
        ...
        public JsonResult List(string sidx, string sord, int page, int rows, bool _search, string searchField, string searchOper, string searchString)
        {
            ...
            var _menuList = from mn in db.pos_menu_application
                            where (mn.deleted_date == null)
                            join md in db.pos_module_application on mn.module_id equals md.module_id
                            join u in db.pos_user_application on mn.updated_by equals u.user_id
                            into t_joined
        ...
        public JsonResult GetActiveMenu(int user_type_id, int job_level_id)
        {
            ...
            var _qry = from t in db.pos_menu_application
                ...
                switch (Request.Form["oper"])
                {
                    case "del":
                        ...
                        for (int i = 0; i < values.Length; i++)
                        {
                            ...
                            _o = db.pos_menu_application.Find(id);
                            _o.fl_active = false;
        ...
        {
            pos_menu_application _o = db.pos_menu_application.Find(id);
            db.pos_menu_application.Remove(_o);
            db.SaveChanges();
