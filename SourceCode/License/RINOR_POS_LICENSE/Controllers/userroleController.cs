...
namespace RINOR_POS.Controllers
{
    ...
    public class userroleController : Controller
    {
        ...
        public ActionResult Setting(int? id)
        {
            ...
            List<vw_menu_app_list> exist = (from a in db.vw_menu_app_list.Where(a => a.UserRoleID == id)
                                            select a).ToList<vw_menu_app_list>();
            if (exist.Count == 0)
                id = 0;
            ...
            userroleViewModel roleUserView = new userroleViewModel()
            {
                ...
                menu_list = (from a in db.vw_menu_app_list.Where(a => a.UserRoleID == id)
                                 select a).ToList<vw_menu_app_list>()
            };
